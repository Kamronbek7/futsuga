...
// URL’ga GET so‘rov yuborib JSON qaytaradigan funksiya
int fetch_and_parse_json(const char *url) {
    int n = 0;
    printf("%d", ++n);
    CURL *curl = NULL;
    printf("%d", ++n);
    ...
    // Sertifikatni memorydan berish
    struct curl_blob blob;
    printf("%d", ++n);
    blob.data = (void *)my_ca_cert;
    printf("%d", ++n);
    blob.len = strlen(my_ca_cert);
    printf("%d", ++n);
    ...
    curl_easy_setopt(curl, CURLOPT_FAILONERROR, 1L); // 4xx/5xx xatolarni ham xato qilib qaytaradi
    ...
    if (!json) {
        ...
        fprintf(stderr, "Received payload (first 1024 bytes):\n%.*s\n", (int) (chunk.size < 1024 ? chunk.size : 1024), chunk.data);
        curl_easy_cleanup(curl);
        free(chunk.data);
        return 1;
    }
